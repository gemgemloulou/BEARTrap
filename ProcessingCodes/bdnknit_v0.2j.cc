/*
  ...
  bdnknit_v0.2i.cc (bswang, 5/31/2016): Moved lines involving the variable "p2" from outside the simion do loop to inside the do loop.  This resolves the issue of certain simion variables (vel, phi, theta, ke, ke_err) being written incorrectly to the root file for the first event.    

  bdnknit_v0.2j.cc (bswang, 6/3/2016): Added statements at the end of the while loop that re-initialize some of the arrays (pid, de_l, e_l, de_b, e_b, g_E, g_px, g_py, g_pz, ce_E, ce_px, ce_py, ce_pz).  This fixes the issue of values from one event sometimes being written to the following event as well.    
*/
#include "stdio.h"
#include "string.h"
#include "TFile.h"
...
/*
    //bswang, 5/31/2016: Moved these 9 lines to the "do" loop below.
    p2 = strpbrk(str2,"\t");  //locate pointer for pointer to first comma
    if(iverb) fprintf(stderr,"p2:%i\n",p2);  //bswang, 5/31/2016
...
*/
    ...
    do{
      ...
      //bswang, 5/31/2016: Moved these 9 lines from above to here 
      p2 = strpbrk(str2,"\t");  //locate pointer for pointer to first comma
      if(iverb) fprintf(stderr,"p2:%i\n",p2);  //bswang, 5/31/2016
      while(p2!=NULL){      //Now loop over every character
        d2[ind2]=p2-str2;  //assign relative index to array   
    ...
    //Re-initializing arrays (bswang, 6/3/2016)
    for(int i=0;i<(MAXG+MAXCE+1);++i) g.pid[i] = g.de_b[i] = g.e_b[i] = g.de_l[i] = g.e_l[i] = 0;
    for(int i=0;i<(MAXG);++i) m.g_E[i] = m.g_px[i] = m.g_py[i] = m.g_pz[i] = 0;
    for(int i=0;i<(MAXCE);++i) m.ce_E[i] = m.ce_px[i] = m.ce_py[i] = m.ce_pz[i] = 0;
